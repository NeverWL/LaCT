...
# Import TTT operations from lact_llm
from lact_model.ttt_operation import block_causal_lact_swiglu, prenorm_block_causal_lact_swiglu, l2_norm
...
try:
    ...
    warnings.warn(
        "Flash Attention is not installed. Falling back to standard PyTorch attention. "
        "For better performance, install via `pip install flash-attn --no-build-isolation`",
...
class LaCTASRLayer(nn.Module):
    """
    LaCT layer adapted for ASR tasks.
    This layer combines sliding window attention with fast weight updates optimized for audio sequences.
    """
    ...
    def __init__(
        ...
        audio_adapt: bool = True,  # Enable audio-specific adaptations
        temporal_reduction: int = 1,  # Factor to reduce temporal resolution
    ):
        super().__init__()
        ...
        # ASR-specific adaptations
        self.audio_adapt = audio_adapt
        self.temporal_reduction = temporal_reduction
        
        if self.audio_adapt and self.temporal_reduction > 1:
            # Temporal downsampling for audio sequences
            self.temporal_downsample = nn.Conv1d(
                hidden_size, hidden_size, 
                kernel_size=temporal_reduction, 
                stride=temporal_reduction,
                groups=hidden_size
            )
    ...
    def forward(
        ...
        **kwargs,
        ...
        # Apply temporal downsampling for audio if enabled
        if self.audio_adapt and self.temporal_reduction > 1:
            # Transpose for conv1d: [b, s, d] -> [b, d, s]
            hidden_states_ds = hidden_states.transpose(1, 2)
            hidden_states_ds = self.temporal_downsample(hidden_states_ds)
            # Transpose back: [b, d, s'] -> [b, s', d]
            hidden_states_ds = hidden_states_ds.transpose(1, 2)
            q_len = hidden_states_ds.size(1)
        else:
            hidden_states_ds = hidden_states
        ...
        if self.use_momentum:
            momentum = self.momentum_proj(hidden_states_ds) # [b, s, nh]
        ...
        if self.learnable_ttt_scale: 
            ttt_scale = F.silu(self.ttt_scale_proj(hidden_states_ds), inplace=False)
        ...
        if self.audio_adapt and self.temporal_reduction > 1:
            ...
            ttt_x_normed = ttt_x_normed[:, :hidden_states.size(1), :]
