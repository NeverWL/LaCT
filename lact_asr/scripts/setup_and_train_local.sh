#!/bin/bash
#
# Complete setup and training script for LaCT ASR with LibriSpeech (Local Version)
...
DEFAULT_DATA_DIR="/tmp/LibriSpeech_LaCT"
...
Complete setup and training pipeline for LaCT ASR with LibriSpeech (Local Version).
This script will:
...
date
echo "🚀 Starting LaCT ASR Setup and Training Pipeline (Local)"
echo "Working directory: $(pwd)"
echo "Virtual environment: $VIRTUAL_ENV"
if command -v nvidia-smi >/dev/null 2>&1; then
    nvidia-smi
else
    echo "nvidia-smi not available - running on CPU or non-NVIDIA GPU"
fi
echo ""

print_status "LaCT ASR Setup and Training Pipeline (Local)"
print_status "============================================="
...
if [[ -z "$VIRTUAL_ENV" ]]; then
    ...
    print_status "It's recommended to use a virtual environment for training"
fi
